...
impl ErrorMetricsCollector {
    ...
    pub fn generate_prometheus_metrics(&self) -> String {
        ...
        output.push_str("# HELP logfusion_errors_total Total number of errors by type\n");
        ...
        output
            .push_str("\n# HELP logfusion_errors_by_level_total Total number of errors by level\n");
        ...
        output.push_str(
            "\n# HELP logfusion_errors_by_target_total Total number of errors by target\n",
        );
        output.push_str("# TYPE logfusion_errors_by_target_total counter\n");
        for (target, count) in &self.target_counts {
            output.push_str(&format!(
                "logfusion_errors_by_target_total{{target=\"{}\"}} {}\n",
                target, count
            ));
        }
...
fn main() {
    // Initialize LogFusion for automatic error logging
    logfusion::info!("Starting error monitoring dashboard demo");
    ...
    logfusion::info!("Error monitoring dashboard demo completed");
}
