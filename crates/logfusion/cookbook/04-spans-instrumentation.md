# Spans and Instrumentation with Tracing via LogFusion

This guide covers tracing's span functionality accessed through LogFusion, including manual span creation and the `#[instrument]` attribute for automatic function tracing. Spans provide hierarchical context for operations and are essential for distributed tracing and performance monitoring.

**What LogFusion provides:** Direct access to tracing's span macros and instrumentation features, plus convenient re-exports of tracing components.
...
```rust
use logfusion::{span, debug, info, Level};
...
```rust
use logfusion::{info, instrument, Level};
...
```rust
use logfusion::{info, span, instrument, Level};
...
```rust
use logfusion::{info, warn, instrument};
...
```rust
use logfusion::{instrument, Level};
...
```rust
use logfusion::{instrument, error};
