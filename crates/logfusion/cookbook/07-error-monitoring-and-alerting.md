...
This cookbook entry demonstrates how to build comprehensive error monitoring and alerting systems using LogFusion's `error_info()` method for structured error introspection.
...
impl ErrorMetrics {
    fn generate_prometheus_metrics(&self) -> String {
        ...
        output.push_str("# HELP logfusion_errors_total Total errors by type\n");
        output.push_str("# TYPE logfusion_errors_total counter\n");
        for (error_code, count) in &self.error_counts {
            output.push_str(&format!(
                "logfusion_errors_total{{error_code=\"{}\"}} {}\n", 
                error_code, count
            ));
        }
        
        // Error counts by level
        output.push_str("# HELP logfusion_errors_by_level_total Total errors by severity\n");
        output.push_str("# TYPE logfusion_errors_by_level_total counter\n");
        for (level, count) in &self.level_counts {
            output.push_str(&format!(
                "logfusion_errors_by_level_total{{level=\"{}\"}} {}\n",
                level, count
            ));
        }
...
{
  "dashboard": {
    "title": "LogFusion Error Monitoring",
    "panels": [
      {
        ...
        "targets": [
          {
            "expr": "rate(logfusion_errors_by_level_total[5m])",
      ...
      },
      {
        ...
        "targets": [
          {
            "expr": "topk(10, logfusion_errors_total)",
      ...
      },
      {
        ...
        "targets": [
          {
            "expr": "increase(logfusion_errors_total[1m])",
            "legendFormat": "{{error_code}}"
          }
        ]
      }
...
# grafana-alerts.yml
groups:
  - name: logfusion-errors
    rules:
      - alert: HighErrorRate
        expr: rate(logfusion_errors_by_level_total{level="error"}[5m]) > 0.1
        for: 2m
        labels:
          severity: critical
        annotations:
      ...
      - alert: DatabasePerformance
        expr: rate(logfusion_errors_total{error_code="DatabaseTimeout"}[5m]) > 0.05
        for: 1m
        labels:
          severity: warning
        annotations:
