...
This guide covers advanced tracing ecosystem integration using LogFusion, including OpenTelemetry, custom subscribers, and integration with monitoring platforms. LogFusion provides convenient access to the full tracing ecosystem while tracing handles the underlying observability infrastructure.

**What LogFusion provides:** Convenient re-exports of tracing-subscriber components, simplified initialization, and direct access to tracing's advanced features.
...
```rust
use logfusion::{info, instrument, registry, SubscriberExt, SubscriberInitExt};
...
async fn setup_opentelemetry() -> Result<(), Box<dyn std::error::Error>> {
    ...
    registry()
        ...
        .with(logfusion::fmt::layer().with_target(false))
        .with(logfusion::EnvFilter::from_default_env())
        .init();
...
impl Default for OtelConfig {
    fn default() -> Self {
        Self {
            service_name: "logfusion-service".to_string(),
    ...
    registry()
        ...
        .with(
            logfusion::fmt::layer()
                ...
                .with_span_events(logfusion::fmt::format::FmtSpan::ACTIVE)
        )
        .with(logfusion::EnvFilter::from_default_env())
        .init();
...
```rust
use logfusion::{registry, SubscriberExt, SubscriberInitExt, info};
...
```rust
use logfusion::{info, registry, fmt, SubscriberExt, SubscriberInitExt, EnvFilter};
...
```rust
use logfusion::{registry, fmt, SubscriberExt, SubscriberInitExt, EnvFilter};
...
```rust
use logfusion::{info, instrument, registry, fmt, SubscriberExt, SubscriberInitExt};
...
```rust
use logfusion::{debug, trace, info};
...
#[cfg(test)]
mod test_tracing {
    use logfusion::{registry, fmt, SubscriberExt, SubscriberInitExt, info};
    use std::sync::Once;
    ...
    pub fn init_test_tracing() {
        INIT.call_once(|| {
            registry()
                ...
                .with(logfusion::EnvFilter::new("debug"))
                .init();
        });
    }
