...
#[test]
fn test_error_info_logffi_format() {
    define_errors! {
        LogFFIError {
    ...
    let err = LogFFIError::SimpleError;
    let (code, level, target) = err.error_info();
    assert_eq!(code, "SimpleError");
    assert_eq!(level, "error");
    assert!(target.contains("error_info_method"));
    ...
    let err = LogFFIError::DatabaseError {
    ...
    let err = LogFFIError::ValidationWarning {
    ...
    let err = LogFFIError::InfoMessage;
    let (code, level, target) = err.error_info();
    ...
    // Test defaults
    let err = LogFFIError::DefaultError;
    let (code, level, target) = err.error_info();
