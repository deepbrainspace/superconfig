//! Tests for LogFFI's structured logging syntax
//!
//! These tests verify that LogFFI macros support structured field syntax
//! and that the re-exported tracing-subscriber components work correctly.

use logffi::{EnvFilter, SubscriberExt, SubscriberInitExt, fmt, registry};
...
#[test]
fn logffi_structured_logging_all_levels() {
    let (collector, events) = StructuredCollector::new();
    let _guard = registry().with(collector).set_default();
    events.lock().unwrap().clear();
...
#[test]
fn logffi_structured_logging_with_targets() {
    let (collector, events) = StructuredCollector::new();
    let _guard = registry().with(collector).set_default();
    events.lock().unwrap().clear();
...
#[test]
fn logffi_structured_logging_mixed_types() {
    let (collector, events) = StructuredCollector::new();
    let _guard = registry().with(collector).set_default();
    events.lock().unwrap().clear();
...
#[test]
fn logffi_reexports_work() {
    // Test that LogFFI's re-exports of tracing-subscriber work correctly
    let (collector, events) = StructuredCollector::new();

    // This should work using only LogFFI re-exports
    let _guard = registry()
        .with(collector)
...
#[test]
fn logffi_json_output_configuration() {
    ...
    // Set up JSON formatting using LogFFI re-exports
    let _guard = registry()
...
#[test]
fn logffi_backwards_compatibility_with_simple_syntax() {
    let (collector, events) = StructuredCollector::new();
    let _guard = registry().with(collector).set_default();
    events.lock().unwrap().clear();
