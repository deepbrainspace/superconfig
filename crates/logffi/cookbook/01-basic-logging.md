...
This guide covers the fundamentals of logging with tracing through LogFFI, including LogFFI's auto-initialization feature, access to tracing's logging macros, and environment configuration.
...
LogFFI provides a simple, zero-configuration way to access the modern `tracing` ecosystem's logging capabilities.

**What LogFFI provides:** Auto-initialization, convenient macros, and re-exported tracing-subscriber components.
...
fn main() {
    // No initialization needed - LogFFI handles it automatically!
...
That's it! LogFFI's auto-initialization feature sets up tracing with sensible defaults when you use any logging macro.

## Auto-Initialization

LogFFI's auto-initialization feature means you can start logging immediately without boilerplate setup code.
...
fn main() {
    // First call to any LogFFI macro automatically initializes tracing
...
fn main() {
    // This is equivalent to what LogFFI does automatically:
    /*
...
LogFFI provides access to tracing's five logging levels, each corresponding to a different severity:

```rust
use logffi::{trace, debug, info, warn, error};
...
LogFFI respects the standard `RUST_LOG` environment variable for controlling log levels:
...
LogFFI supports Rust's standard formatting syntax:
...
LogFFI is built on `tracing`, so it integrates seamlessly with the entire ecosystem:
...
fn mixed_usage() {
    // LogFFI macros and tracing macros work together
    info!("Using LogFFI macro");
...
LogFFI automatically works with libraries using the `log` crate:

```rust
use logffi::info;

fn main() {
    // LogFFI automatically sets up log-to-tracing bridge
    info!("LogFFI message");
...
```rust
use logffi::{info, registry, fmt, EnvFilter, SubscriberExt, SubscriberInitExt};

fn main() {
    // Set up custom tracing subscriber BEFORE using LogFFI macros
    // Using tracing-subscriber components re-exported by LogFFI for convenience
    registry()
        .with(fmt::layer().json())
        .with(EnvFilter::from_default_env())
        .init();
    
    // Now LogFFI macros will use your custom tracing setup
...
```rust
use logffi::{info, warn, error, debug};
...
```rust
// Set up JSON formatting before using LogFFI - using tracing-subscriber via LogFFI's re-exports
use logffi::{info, registry, fmt, SubscriberInitExt};
...
mod tests {
    ...
    fn test_with_logging() {
        // This ensures tracing logs appear during testing - using LogFFI's re-exported components
        let _ = logffi::fmt::try_init();
        
        logffi::info!("Test log message");
        // Your test code
    }
}
```
