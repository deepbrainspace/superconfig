...
This guide covers LogFFI's Foreign Function Interface (FFI) capabilities and callback-based logging. LogFFI provides seamless integration with C libraries, callback functions, and cross-language logging scenarios while tracing handles the underlying observability infrastructure.

**What LogFFI provides:** FFI-safe logging macros, callback registration system, C-compatible interfaces, and auto-initialization across language boundaries.
...
```rust
use logffi::{info, warn, error, ffi_info, ffi_warn, ffi_error};
...
// Initialize LogFFI for FFI usage
#[no_mangle]
pub extern "C" fn logffi_init() -> c_int {
    // LogFFI auto-initialization will happen on first log call
    // This function can be used for explicit initialization
    info!("LogFFI initialized for FFI usage");
    0 // Success
}

// Shutdown function for clean FFI teardown
#[no_mangle]
pub extern "C" fn logffi_shutdown() {
    info!("LogFFI shutting down");
...
```rust
use logffi::{info, error, instrument};
use std::ffi::{CStr, CString};
use std::os::raw::{c_char, c_int, c_void};
...
```rust
use logffi::{error, define_errors};
use std::ffi::{CStr, CString};
use std::os::raw::{c_char, c_int};
use std::sync::Mutex;
...
You now have comprehensive knowledge of LogFFI's capabilities. Consider exploring:
