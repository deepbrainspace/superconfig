//! LogFFI Format Showcase
//!
//! This example demonstrates the new LogFFI format introduced in v0.2, featuring:
...
//!
//! Run with:
//! cargo run --example logffi_format_showcase
//!
//! Or with different log levels:
//! RUST_LOG=debug cargo run --example logffi_format_showcase
//! RUST_LOG=trace cargo run --example logffi_format_showcase

use logffi::define_errors;
use std::error::Error;

fn main() {
    println!("🆕 LogFFI Format Showcase (v0.2)");
    println!("=================================");
    println!("Demonstrating the new clean LogFFI syntax!\n");

    // Auto-initialization happens on first log call
    demonstrate_basic_logffi_format();
    ...
    println!("\n✨ LogFFI v0.2 - The future of Rust error handling!");
}

fn demonstrate_basic_logffi_format() {
    println!("📦 Basic LogFFI Format");
    println!("----------------------");
...
fn demonstrate_mixed_variants() {
    ...
    // Mix unit and struct variants in the same enum - this is unique to LogFFI format!
    define_errors! {
        MixedError {
...
fn demonstrate_multiple_error_types() {
    ...
    // Define multiple error enums in one macro call - another LogFFI exclusive feature!
    define_errors! {
        ApiError {
...
fn demonstrate_real_world_example() {
    ...
    // Comprehensive e-commerce error handling with LogFFI format
    define_errors! {
        PaymentError {
