//! LogFFI Source Chaining - Automatic Detection
//!
//! This example demonstrates LogFFI v0.2's automatic source chaining feature:
...
//!
//! Run with:
//! cargo run --example logffi_source_chaining
//!
//! Or with debug logs:
//! RUST_LOG=debug cargo run --example logffi_source_chaining

use logffi::define_errors;
use std::error::Error;

fn main() {
    println!("⛓️ LogFFI Automatic Source Chaining Example");
    println!("=============================================");
    println!("Demonstrating automatic source detection for fields named 'source'\n");

    demonstrate_logffi_auto_source_chaining();
    demonstrate_traditional_source_chaining();
    demonstrate_mixed_source_types();
    show_source_chain_walking();

    println!("\n🎯 LogFFI Auto Source Chaining Benefits:");
    ...
    println!("\n✨ LogFFI makes error chaining effortless!");
}

fn demonstrate_logffi_auto_source_chaining() {
    println!("🆕 LogFFI Format - Automatic Source Detection");
    println!("----------------------------------------------");

    // 🆕 LogFFI Format - Fields named "source" automatically become #[source]
    define_errors! {
        LogFFIChainedError {
    ...
    let io_failure = LogFFIChainedError::IoFailure {
    ...
    let net_err = LogFFIChainedError::NetworkError {
    ...
    let parse_err = LogFFIChainedError::ConfigParseError {
    ...
    let payment_err = LogFFIChainedError::PaymentProcessingError {
