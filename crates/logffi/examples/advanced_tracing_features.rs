...
fn demonstrate_log_crate_bridge() {
    ...
    // Mixed with our LogFFI macros
    error!("LogFFI error message");
    warn!("LogFFI warning message");
    info!("LogFFI info message");
    debug!("LogFFI debug message");
}
...
async fn main() {
    ...
    info!("✅ Error integration - thiserror + LogFFI + tracing");
